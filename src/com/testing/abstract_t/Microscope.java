...
/**
 * Created by zhangjiyun on 2017/3/16.
 */
/*

   Originally written by Doug Lea and released to the public domain.

   file: Microscope.java
   author: Doug Lea
   history:
   initial version: Mon Sep  4 20:19:46 1995  Doug Lea  (dl at gee)
      (Note that the display code was written using pre-1.0 alpha
       java so is very primitive!)
   ...
   Wed Feb 14 06:47:27 1996  Doug Lea  (dl at gee)
       ...
       correctly.
   Fri Jun  6 13:38:39 1997  Doug Lea  (dl at gee)
       Replaced suspend with wait
   Fri Jan  2 14:25:57 1998  Doug Lea  (dl at gee)
       replaced stop with interrupt; recompiled with 1.2b2
*/
...
public final class Microscope extends java.applet.Applet  {
...
//    resize((Board.RANKS + 3) * CELL_SIZE, (Board.RANKS + 1) * CELL_SIZE);
    ...
    public synchronized void move(Move m, Mover mvr) {
        if (mvr == mover && m.isLegal()) {
            m.commit();
            board = m.board();
    ...
    public boolean mouseUp (Event evt, int x, int y) {
        ...
        if (Board.inBounds(row, col)) { // cell selection
            ...
            switch (col) {
                case 0:  break;
                case 1: repaint(); findOrCancel(); break;
...
final class User extends Mover {
...
final class AutoMover extends Mover implements Runnable {
    ...
    synchronized void setCancelled(boolean b) { cancelled = b; }

    public synchronized void changeLevel(int incr) {
        lookAheads += incr;
    ...
    int findMove(Board source, Player turn, int level) {
        ...
        if (Thread.interrupted() && isCancelled()) return NOMOVE;
